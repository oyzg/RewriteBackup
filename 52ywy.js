...
[rewrite_local]
^https:\/\/miniapp2\.52ywy\.com\/(ywyminiappbot\/(getLatest|getLatestAll|getCategoryVideos|getSearch|series|movies|tv|av|getbloger)|player\/getPlayer)(\?|$) url script-response-body https://raw.githubusercontent.com/oyzg/RewriteBackup/refs/heads/main/52ywy.js
...
!(async () => {
  ...
  let data;
  try { data = JSON.parse(body); } catch (_) { return $.done({ body }); }

  if (LIST_RE.test(url) && Array.isArray(data)) {
    const { map, evicted } = pruneExpired($.getjson(STORE_KEY, {}));
    let added = 0;
    for (const it of data) {
      if (it && it.id != null && it.r2) {
        map[String(it.id)] = { r2: it.r2, t: Date.now() };
        added++;
      }
    }
    if (added || evicted) $.setjson(map, STORE_KEY);
    walkZero(data);
    $.log(`列表缓存 +${added} 项，过期清理 ${evicted} 项，共 ${Object.keys(map).length} 项`);
    return $.done({ body: JSON.stringify(data) });
  }

  if (PLAYER_RE.test(url)) {
    const m = url.match(/videoid=(\d+)/);
    if (m && data && typeof data === "object" && !Array.isArray(data)) {
      const vid = m[1];
      let r2 = null;
      const dict = await loadRemoteDict();
      if (dict && dict[vid]) r2 = dict[vid];
      if (!r2) {
...
function walkZero(node) {
  if (Array.isArray(node)) {
    for (const v of node) walkZero(v);
  } else if (node && typeof node === "object") {
    for (const k of Object.keys(node)) {
      if (KEYS_ZERO.has(k) && typeof node[k] === "number") node[k] = 0;
      else walkZero(node[k]);
    }
  }
}

async function loadRemoteDict() {
  const cached = $.getjson(DICT_KEY, null);
  if (cached && cached.data && Date.now() - cached.t < TTL_MS) return cached.data;
  try {
    const resp = await $.http.get({ url: GIST_URL, timeout: 10000 });
    const dict = $.toObj(resp.body);
    if (dict && typeof dict === "object" && !Array.isArray(dict)) {
      $.setjson({ data: dict, t: Date.now() }, DICT_KEY);
      $.log(`字典刷新 ${Object.keys(dict).length} 条`);
      return dict;
    }
  } catch (e) {
...
function pruneExpired(raw) {
  const map = {}; let evicted = 0; const now = Date.now();
  for (const k of Object.keys(raw)) {
    const v = raw[k];
    if (v && typeof v === "object" && v.r2 && now - v.t < TTL_MS) map[k] = v;
    else evicted++;
  }
  return { map, evicted };
}
